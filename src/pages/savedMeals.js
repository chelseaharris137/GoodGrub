    ...
    <div className={classes.pageWrapper}>
      ...
      <div className={classes.list_container}>
        ...
        {result && result.map(({ data, isLoading }, index) => {
          if (isLoading) {
            return (
              <BeatLoader color="#fff" key={savedMealsId[[index]]} loading={isLoading} size={20} />
            );
          }

          return (
            <Link className={classes.singleMeal} href={`/meals/${data.idMeal}`} key={data.idMeal}>
              <Title className={classes.mealTitle} variant="secondary">{data.strMeal}</Title>
